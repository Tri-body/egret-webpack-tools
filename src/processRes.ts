...
export async function processRes(compiler: Compiler, compilation: compilation.Compilation, options: IOptions) {
  ...
  const resJson = await fs.readJson(path.join(resPath, DEFAULT_RES_NAME))
  ...
  for (let r of resources) {
    ...
    if (r.type !== 'sheet') {
      ...
      outputName2 = toAssetsByOutputName(compilation, buffer, fileName2, getOutputName(fileName2, resBaseName))
